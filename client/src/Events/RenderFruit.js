...
const RenderFruit = (props) => {
  ...
  useEffect(() => {
    ...
    ctx.drawImage(
      ...
      0,
      0,
      image.naturalWidth * 1.5,
      image.naturalHeight * 1.5
    );
    ...
    for (let i = 0; i < number; i++) {
      ...
      ctx.drawImage(
        ...
        x,
        y,
        catImage.naturalWidth / 10,
        catImage.naturalHeight / 10
      );
    }
    writeText(
      { text: number, x: image.naturalWidth * 1.5 - 100, y: 10 },
      { fontSize: 30, color: "green", textAlign: "center" }
    );
    ctx.drawImage(
      catImage,
      image.naturalWidth * 1.5 - 60,
      -3,
      catImage.naturalWidth / 10,
      catImage.naturalHeight / 10
    );
